...
- [ ] High performance compute (HPC) Standard_H16r
...
**Storage type:**
- [ ] An Azure Data Lake Storage Gen2
- [ ] An Azure Block blobs
...
- **A.** Create a general-purpose v1 storage account
- **B.** Create a blob container and copy the files to the blob container.
- **C.** Create a general-purpose v2 storage account that is configured for the Hot default access tier
- **D.** Create a blob container, copy the files to the blob container, and set each file to the Archive access tier.
- **E.** Create a general-purpose v2 storage account that is configured for the Cool default access tier
- **F.** Create a file share in the storage account and copy the files to the file share.
...
- [ ] An Active Directory Application proxy Connector.
...
- [ ] An Azure Web Application Firewall (WAF) in azure Front Door
...
**To enforce Azure MFA authentification, configure:**
- [ ] Grant control in capolicy1.
- [ ] Session control in capolicy1.
- [ ] A sign-in risk policy in Azure AD Identity Protection for the Litware.com tenant.
...
### Question 75
You have an Azure Load Balancer named LB1 that balances requests to five Azure virtual machines. You need to develop a monitoring solution for LB1. The solution must generate an alert when any of the following conditions are met:
- A virtual machine is unavailable.
- Connection attempts exceed 50,000 per minute.

Which signal should you include in the solution for each condition?

**An unavailable virtual machine:**
- [ ] Byte count
...
**More than 50,000 connection attempts per minute:**
- [ ] Byte count
- [ ] Data path availability
- [ ] Health probe status
- [ ] Packet count
...
**Explanation:**
Data path availability monitors the health of the load balancer's data path and detects when backend instances become unavailable. SYN count measures TCP connection attempts, providing visibility into connection request volume that can trigger alerts when exceeding thresholds.

Standard Load Balancer continuously exercises the data path from within a region to validate connectivity to backend instances. The SYN count metric reports the number of TCP SYN packets received, indicating connection attempt volume.
</details>

---

### Question 76
You have an Azure Active Directory (Azure AD) tenant. You plan to use Azure Monitor to monitor user sign-ins and generate alerts based on specific user sign-in events.
...
**Send Azure AD logs to:** An Azure Log Analytics workspace
**Signal type to use for triggering the alerts:** Log

**Explanation:**
Azure AD logs must be sent to Log Analytics workspace to enable log-based alerts. Log signal type allows creating alerts based on log queries that analyze user sign-in events and patterns.

Log Analytics provides the query capabilities needed to analyze sign-in patterns and trigger alerts based on specific conditions in the Azure AD sign-in logs.
</details>
...
You need to design an Azure governance solution that must meet the following requirements:
...
**Level at which to create the blueprint assignments:** Each child management group

**Explanation:**
Define blueprints at the root management group level to ensure consistency across all child management groups and subscriptions. Assign blueprints at each child management group level to apply governance consistently while minimizing the number of assignments.

This approach provides centralized blueprint definition with distributed assignment, maintaining consistency while optimizing management overhead.
</details>

---

### Question 78
You need to configure an Azure policy to ensure that Azure SQL databases have TDE enabled. The solution must meet security and compliance requirements.
...
**Explanation:**
The deployIfNotExists effect automatically enables TDE on non-compliant databases. A managed identity provides the necessary permissions for the policy to modify resources. The remediation task applies the policy to existing resources to bring them into compliance.

This sequence ensures automated enforcement of TDE encryption across all SQL databases while maintaining security and compliance requirements.
</details>
...
- **A.** Azure Policy with audit effects
- **B.** Azure Policy with deployIfNotExists effects
- **C.** Azure Blueprints
...
- **A.** Byte Count
- **B.** Data Path Availability
- **C.** Health Probe Status
- **D.** Packet Count
- **E.** SYN Count
...
**Explanation:**
- **Sales:** Azure Site Recovery provides failover to secondary location with minimal RTO, meeting the requirement to fail over to another data center.
- **Finance:** Azure Site Recovery provides 10-minute RTO for application availability, while Azure Backup ensures 7-year data retention for compliance.
- **Reporting:** Azure Backup meets the 8-hour RTO and daily granularity requirements at lower cost than ASR.

This combination provides cost-effective disaster recovery while meeting each application's specific RTO and retention requirements.
</details>
...
You add a new data factory. What should you do next?
...
**Explanation:**
The self-hosted integration runtime enables secure connectivity between Azure Data Factory and on-premises resources. Creating a pipeline defines the data movement workflow from Server1 to Azure Storage.

The Integration Runtime provides the data integration infrastructure, while pipelines orchestrate the data movement and transformation processes between source and destination.
</details>
...
2. **Azure AD Enterprise Application**
3. **Azure AD Conditional Access policy**

**Explanation:**
- **Azure AD Application Proxy** provides secure remote access to on-premises applications.
- **Azure AD Enterprise Application** enables integration and single sign-on configuration.
- **Azure AD Conditional Access policy** enforces MFA requirements based on location and risk factors.

This sequence provides secure external access with MFA enforcement for on-premises applications using Azure AD authentication.
</details>
...
You create an Azure SQL database named DB1 that is hosted in the East US region. To DB1, you add a diagnostic setting named Settings1. Settings1 archives SQLInsights to storage1 and sends SQLInsights to Workspace1.
...
**Explanation:**
Azure SQL Database diagnostic settings support multiple destinations simultaneously. You can configure additional diagnostic settings to send the same diagnostic data to different storage accounts, Log Analytics workspaces, or Event Hubs without conflicts.

Each diagnostic setting operates independently, allowing multiple configurations for the same resource and diagnostic category.
</details>
...
- **A.** Replication policy with 15-minute RPO, secondary region, standard storage
- **B.** Replication policy with 5-minute RPO, secondary region, premium storage
- **C.** Replication policy with 30-minute RPO, same region, standard storage
- **D.** No replication, only backups
...
- **A.** Replication policy with 5-minute RPO, secondary region, premium storage
- **B.** Replication policy with 15-minute RPO, secondary region, standard storage
- **C.** Replication policy with 30-minute RPO, same region, standard storage
- **D.** No replication, only backups
...
| Storage types|
|--------------|
| A geo-redundant sotrage (GRS) account|
| A locally-redundant sotrage (LRS) account|
